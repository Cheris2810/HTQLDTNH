...
// Truy vấn lấy dữ liệu từ bảng monan với điều kiện tìm kiếm, lọc loại món và MaChayMan = 1
$sql = "SELECT monan.ma_ma, monan.ten_ma, monan.hinh_anh, loaimon.ten_lm 
        FROM monan 
        LEFT JOIN loaimon ON monan.ma_loaimon = loaimon.ma_lm 
        WHERE monan.MaChayMan = 2 $search_condition $filter_condition 
...
// Truy vấn tổng số món ăn với MaChayMan = 1
$total_sql = "SELECT COUNT(*) AS total FROM monan WHERE MaChayMan = 2 $search_condition";
...
<body style="margin-top: 4rem;">
    ...
    <div class="left" style="margin-left:1rem">
        <form action="connectmonman.php" method="POST" enctype="multipart/form-data" id="monanForm">
            ...
            <img id="previewImg" src="" alt="Xem trước hình ảnh" style="display:none; width: 100%; margin-top: 10px; border-radius: 4px; width: 2rem;"/>

            <!-- Thêm lựa chọn món chay/mặn -->
            <div style="display:flex; margin-top: -1rem;">
                <div >
                    <input style="margin-left: 4rem" type="radio" id="monchay" name="ma_chayman" value="1" required>
                    <label for="monchay" style="margin-left: 2rem">Món chay</label>
                </div>
                <div >
                    <input style="margin-left: 4rem" type="radio" id="monman" name="ma_chayman" value="2" required>
    ...
    <!-- Danh sách món ăn -->
    <div class="right" style="height: 119rem;">
                ...
                <div>
                    ...
                    <div>
                        <select name="filter_nhommon" id="filter_loaimon" onchange="this.form.submit()">
                            ...
                            <?php
                            // Lấy danh sách loại món từ cơ sở dữ liệu chỉ với MaChayMan = 2
                            $loaimon_sql = "SELECT ma_lm, ten_lm FROM loaimon WHERE MaChayMan = 2";
                            $loaimon_result = $conn->query($loaimon_sql);
    ...
    <h1 style="color:black; margin-top: -0.5rem;">Danh sách món mặn</h1>
    <table>
    <thead>
        <tr>
            <th>Mã món</th>
    ...
    <tbody>
        ...
        // Truy vấn lấy dữ liệu từ bảng monan với điều kiện lọc và MaChayMan = 2
        $sql = "SELECT monan.ma_ma, monan.ten_ma, monan.hinh_anh, loaimon.ten_lm 
                FROM monan 
                LEFT JOIN loaimon ON monan.ma_loaimon = loaimon.ma_lm 
                WHERE monan.MaChayMan = 2 $search_condition $filter_condition";
